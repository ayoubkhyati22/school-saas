...
import { getHomework, createHomework } from '@/services/homework.service';
...
export default function HomeworkPage() {
  ...
  const columns = [
    ...
    },
    {
      key: 'class',
      label: 'Class',
      render: (hw: any) => <span className="text-sm text-foreground">{hw.classes?.name || '—'}</span>,
    },
    {
      key: 'teacher',
      label: 'Teacher',
      render: (hw: any) => <span className="text-sm text-foreground">{hw.profiles?.full_name || '—'}</span>,
    },
    {
      key: 'due_date',
      label: 'Due Date',
  ...
  return (
    <DashboardLayout profile={profile}>
      ...
      <Modal
        ...
        description="Assign homework to a class"
        size="lg"
        footer={
          <>
            <Button variant="outline" onClick={() => setShowModal(false)}>Cancel</Button>
            <Button onClick={handleSubmit} loading={saving}>Create Homework</Button>
          </>
        }
      >
